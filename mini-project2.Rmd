...
author: "Chris Corona, Farshina Nazrul, Gak Roppongi, Nolan Walker"
date: "`r Sys.Date()`"
output: pdf_document
---
...
```{r Data Cleaning, message=FALSE, warning=FALSE, echo=F}
...
```{r Cross Validation Example,eval=FALSE, echo=F}
...
model.onehot.full<-train(price ~ .,data=newdata,method='lm',trControl=train_control) # cross validation training
...
#print(model.onehot.1) # this is the trained model using 10 fold cv from backward selection on one hot encoded data
m1 <- lm(price~., data=data)
train_control <- trainControl(method = "cv",
                              number = 10)
model<-train(price ~ .,data=data,method='lm',trControl=train_control)
#print(model) # this is just the trained model using cross validation without one hot encoding or backward selection
```
...
```{r Lasso Ridge, message=FALSE, warning=FALSE, echo=F, fig.height=3}
...
```{r Ridge, message=FALSE, warning=FALSE, echo=F, fig.height=3}
# RIDGE:::::::::::::::::::::::::::::
...
```{r PCA, message=FALSE, warning=FALSE, echo=F, fig.height=3}
...
```{r Biplot, message=FALSE, warning=FALSE, echo=F}
...
```{r Simple Linear Regression, message=FALSE, warning=FALSE, echo=F, fig.height=3}
library(Metrics)
...
```{r Plot Poly Fit, message=FALSE, warning=FALSE, echo=F}
...
```{r Smoothing Spline, message=FALSE, warning=FALSE, echo=F, fig.height=3}
set.seed(125)
...
```{r Plot Best Smooth Spline, message=FALSE, warning=FALSE, echo=F, fig.height=3}
