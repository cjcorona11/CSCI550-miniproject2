...
Find theme in lasso coefficients - area, year built. Is there a theme in lasso coefficients that were dropped?

Ridge keeps all coefficients. But which has better RMSE: best lasso or best ridge?
```{r lasso_ridge}
...
sprintf("Best Lambda for lasso: %s", round(lambda_out_lasso, digits = 2))
...
sprintf("Best Lambda for ridge: %s", round(lambda_out_ridge, digits = 2))
...
for(i in 1:k) {
  ...
  # calculate PC scores and add to training data frame as new variables
  ...
  mPCA1 <- lm(price~vector[1:i], data=train)
  ...
  pred34 <- predict(mPCA14, newdata=testNewData[,c(1:14)])
  pred35 <- predict(mPCA15, newdata=testNewData[,c(1:15)])
...
## Polynomial
We used the year the house was built as the only predictor to fit various polynomials to the data set. First we split the data set into a train and test set. On the training set, we built 5 models each with `Year.Built` and adding the next order polynomial of `Year.Built` to subsequent models up to 5th order (ie model 1 `price ~ Year.Built`, model 2 `price ~ Year.Built + Year.Built^2`, etc). We used these models to predict on the test set and computed the RMSE. We repeated these steps on 10 folds for k-fold cross-validation. A plot of the cross-validated RMSE for each of the 5 models is shown in Figure.
...
plot(data$Year.Built, data$price)
```
...
smth_spl_mdl = smooth.spline(xtrain , ytrain, lambda = lambdas[which(RMSE_array==min(RMSE_array))])
plot(smth_spl_mdl, type="l", col="red")
points(xtrain, ytrain, col="black", alpha=0.5)
```
